// ChapterCarousel — horizontal swipe through chapters. Touch, mouse, trackpad, keyboard.
// Bottom nav shows prev/next chapter names instead of a generic Prev/Next widget.
...
export function ChapterCarousel({ chapters, activeIndex, onIndexChange }: Props) {
  ...
  // Touch + mouse drag with axis lock.
  useEffect(() => {
    ...
    const start = (x: number, y: number) => { startX = x; startY = y; dragging = true; axis = null }
    ...
    const end = (x: number) => {
      ...
      dragging = false
      const dx = x - startX
      if (axis === 'x' && Math.abs(dx) > SWIPE_THRESHOLD) { if (dx < 0) next(); else prev() }
    }
        ...
        >
          {chapters.map((c) => (
            <div key={c.id} style={{ width: `${100 / chapters.length}%` }} className="h-full">
              <ChapterSlide chapter={c} />
            </div>
          ))}
        </div>
      </div>

      {/* Signal Pink progress bar */}
      <div className="h-0.5 w-full bg-paper-rule">
        <div
          className="h-full bg-signal-pink transition-[width] duration-300"
          style={{ width: `${((activeIndex + 1) / chapters.length) * 100}%` }}
        />
      </div>

      {/* Editorial nav — prev/next chapter names */}
      <nav
        className="flex items-center border-t border-paper-rule bg-paper px-4 py-3 text-xs font-bold"
        aria-label="Chapter navigation"
      >
        <button
          onClick={prev}
          disabled={!prevChapter}
          className="min-w-0 flex-1 truncate text-left text-stone transition-colors hover:text-ink disabled:pointer-events-none disabled:opacity-0"
          aria-label={prevChapter ? `Previous: ${prevChapter.name}` : undefined}
        >
          ← {prevChapter?.name || ''}
        </button>

        <div className="shrink-0 px-4 tabular-nums text-[#9aa0aa]">
        ...
        <button
          ...
          className="min-w-0 flex-1 truncate text-right text-stone transition-colors hover:text-ink disabled:pointer-events-none disabled:opacity-0"
          aria-label={nextChapter ? `Next: ${nextChapter.name}` : undefined}
        >
          {nextChapter?.name || ''} →
        </button>
      </nav>
